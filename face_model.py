...
class Face_Model(Pseudo_Model):
    def __init__(self, device, cfg, use_ddp=False):
        ...
        # Check if the model is configured to use ESRGAN
        self.use_esrgan = cfg.SR.MODEL == "ESRGAN"
        self.sr_warmup_iter = cfg.OPT_SR.WARMUP
        # If using ESRGAN, set up the network architecture and optimizers accordingly
        if self.use_esrgan:
            del self.U  # Delete existing U network
            del self.opt_U
            del self.lr_U
            del self.nets["U"]
            del self.optims["U"]
            ...
            # Add networks and optimizers to respective dictionaries
            self.nets["U"] = self.U
            ...
            # Initialize GAN loss and VGG feature extractor
    ...
    # Check if the training is still in the warm-up phase
    def warmup_checker(self):
        return self.n_iter <= self.sr_warmup_iter

    # Perform a learning rate decay step
    ...
    def train_step(self, Ys, Xs, Yds, Zs):
        ...
        # Enable gradient computation for discriminator networks
        self.net_grad_toggle(["D_x", "D_y", "D_sr"], True)
        # Compute and optimize discriminator losses
        # D_x
        ...
        # Disable gradient computation for discriminator networks
        self.net_grad_toggle(["D_x", "D_y", "D_sr"], False)
        # Compute and optimize generator losses
        # G_yx
        ...
        # Update the super-resolution network (U) if using ESRGAN
        # U
...
if __name__ == "__main__":
    ...
    with open("/Users/noymachluf/Desktop/pseudo-sr/configs/faces.yaml", "rb") as cf:
