...
# Function to initialize weights for a Discriminator
def weight_init_D(m):
    ...
    if class_name.find('Conv') != -1:
        ...
        if hasattr(m.bias, 'data'):
            m.bias.data.fill_(0)
    # If the layer is a 2D BatchNorm
    elif class_name.find('BatchNorm2d') != -1:
        m.weight.data.normal_(1.0, 0.02) # Initialize weights with normal distribution
        m.bias.data.fill_(0) # Initialize bias to zeros

# Class for a PatchGAN discriminator
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, scale_factor=1, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, n_group=1):
        ...
        # Determine if bias should be used based on the normalization layer
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func != nn.BatchNorm2d
        else:
            use_bias = norm_layer != nn.BatchNorm2d
        self.n_layers = n_layers
        # Calculate strides and downscaling factor
        ...
        nf_mult = 1
        nf_mult_prev = 1
        # Add convolutional layers with normalization and activation
    ...
    # Function to compute strides based on scale factor and number of layers
