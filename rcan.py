...
import argparse
# Function to create an RCAN model with specified arguments
...
class CALayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        # Two-layer fully connected network to compute attention scores
    ...
    def forward(self, x):
        """Forward pass for Channel Attention layer."""
        y = self.avg_pool(x)
        y = self.conv_du(y)
        return x * y
...
class RCAB(nn.Module):
    """Initialize an RCAB with two convolutional layers and a CA layer."""
    def __init__(
...
class ResidualGroup(nn.Module):
    """Initialize a Residual Group with multiple RCABs."""
    ...
    def forward(self, x):
        """Forward pass for Residual Group."""
        res = self.body(x)
        res += x
        return res
